  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        if (!session) return;
        const res = await supabaseService.getBookmarkedMaterials(session.user.id);
        if (res && res.success && res.data) setBookmarks(res.data);
      } catch (err) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <FlatList
        ...
        ListEmptyComponent={() => (<View style={styles.empty}><Text style={styles.emptyText}>No bookmarks yet.</Text></View>)}
      />
    </SafeAreaView>
  );
}
