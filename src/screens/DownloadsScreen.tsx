    ...
    const fetch = async () => {
      try {
        ...
        if (res && res.success && res.data) {
          const downloaded = (res.data || []).filter((m: any) => !!(m.local_path || m.is_downloaded));
          setItems(downloaded);
        }
      } catch (err) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <FlatList
        ...
        ListEmptyComponent={() => (<View style={styles.empty}><Text style={styles.emptyText}>No downloads yet.</Text></View>)}
      />
    </SafeAreaView>
  );
}
